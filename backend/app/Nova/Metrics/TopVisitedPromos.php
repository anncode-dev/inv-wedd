...
class TopVisitedPromos extends Partition
{
    /**
    ...
    public function calculate(NovaRequest $request)
    {
        ...
        $promos = Promo::whereBetween(\DB::raw('COALESCE(publish_date, created_at)'), [$startDate, $endDate])
                       ->orderBy('count', 'desc')
                       ->take(10)
                       ->get(['count', 'title']);

        $result = $promos->mapWithKeys(function ($item) {
    ...
    {
        return __('Top 10 Visited Promos (Last 60 Days)');
    }
